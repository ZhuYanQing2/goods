...
<script type="text/javascript">
	$(function(){
		...
		$(":checkbox[name=checkboxBtn]").click(function(){
			var all = $(":checkbox[name=checkboxBtn]").length;//所有条目的数量
			var select = $(":checkbox[name=checkboxBtn][checked=true]").length;//选中条目的数量
			if(all == select){//全部被选中
				$("#selectAll").attr("checked",true);//勾选全选复选框
				setJieSuan(true);//让结算按钮有效
			}else if(select == 0){//全部未选中
				$("#selectAll").attr("checked",false);//取消勾选全选复选框
	...
	function setJieSuan(bool){
		if(bool){
			...
			$("#jiesuan").unbind("click");//撤销当前元素上所有的click时间
		}else{
	...
	function batchDelete(){
		//1.获取所有被选中的复选框
		//2.吧所有被选中的复选框的值添加到数组中
		//3.指定location为cartItemServlet	,参数method=batchDelete，参数cartItemIds=数组中的tostring（）
	...
	//
	function jiesuan(){
		//1.获取所有被选中的条目的id，放入数组
		var cartItemIdArray = new Array();
		$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
			cartItemIdArray.push($(this).val());
		});
		//2.将数组的值tostring，赋值给表单的cartItemIds
...
<c:choose>
	...
	<form id="jieSuanFrom" action="<c:url value='/CartItemServlet'/>" method="post">
